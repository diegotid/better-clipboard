        ...
        Form {
            HStack(alignment: .top) {
                        ...
                        VStack(alignment: .leading) {
                            HStack {
                                ...
                                TextField("", value: $maxHistoryInput, format: .number)
                                    ...
                                    .disabled(!manager.isUnlocked)
                            }
                            .padding(.top, 6)
                            Text(manager.isUnlocked
                                 ? """
                ...
                VStack(alignment: .leading) {
                    ...
                    Spacer()
                    if !manager.isUnlocked {
                        if manager.supportsPurchases {
                            if let price = manager.lifetimePriceDisplay {
                                Button(action: {
                                    Task {
                                        _ = await manager.purchaseLifetimeUnlock()
                                    }
                                }) {
                                    HStack {
                                        Image(systemName: "lock.open")
                                ...
                                .disabled(manager.isLoading || !manager.canPurchaseLifetimeUnlock)
                            ...
                            Text("This source build stays on the free tier. Pro is only available in the App Store release.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                                .fixedSize(horizontal: false, vertical: true)
                                .padding(.top, 8)
