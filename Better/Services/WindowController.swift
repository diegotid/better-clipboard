...
private extension WindowController {
    ...
    func showTranslationWindows(
        ...
        guard await translator.hasInstalledTranslationLanguages() else {
            if !suppressAlerts {
                presentTranslationUnavailableAlert(
    ...
    func restorePurchases(_ sender: Any?) {
        Task {
            let foundEntitlement = await purchaseManager.restorePurchases()
            await MainActor.run {
                let alert = NSAlert()
                if foundEntitlement {
                    ...
                    alert.alertStyle = .informational
                } else if purchaseManager.supportsPurchases {
                    alert.messageText = "No Purchases Found"
                    alert.informativeText = "No previous purchases were found for this Apple ID."
                    alert.alertStyle = .warning
                } else {
                    alert.messageText = "Source Build"
                    alert.informativeText = "This source build stays on the free tier. Pro purchases are only available in the App Store release."
                    alert.alertStyle = .informational
                }
                alert.addButton(withTitle: "OK")
                alert.runModal()
    ...
    func showUpgradeAlert() {
        ...
        if purchaseManager.supportsPurchases {
            ...
            alert.informativeText = "Source builds stay on the free tier and are limited to 3 pinned items."
        }
        alert.alertStyle = .informational
        alert.addButton(withTitle: purchaseManager.supportsPurchases ? "Open Settings" : "OK")
        if purchaseManager.supportsPurchases {
            alert.addButton(withTitle: "Cancel")
        }
        let response = alert.runModal()
        if purchaseManager.supportsPurchases && response == .alertFirstButtonReturn {
            showSettingsPopover()
        }
    }
        ...
        if currentIDs != filteredIDs {
            ...
            for (index, pair) in windows.enumerated() {
                if index < entries.count {
                    ...
                    let canPin = purchaseManager.isUnlocked || pinnedCount < defaultMaxPinnedEntries || updatedEntry.isPinned
                    let newView = AnyView(
                        ClipboardEntry(
                            entry: updatedEntry,
    ...
    func createWindow(for entry: CopiedContent) -> (NSWindow, NSHostingController<AnyView>) {
        let pinnedCount = clipboard.history.filter { $0.isPinned }.count
        let canPin = mode == .history && (purchaseManager.isUnlocked || pinnedCount < defaultMaxPinnedEntries || entry.isPinned)
    ...
    func toggleFrontMostPinned() {
        ...
        if !currentEntry.isPinned {
            let pinnedCount = clipboard.history.filter { $0.isPinned }.count
            let canPin = purchaseManager.isUnlocked || pinnedCount < defaultMaxPinnedEntries
            if !canPin {
